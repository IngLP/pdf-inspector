...
///
/// The box is *not* turned for a page whose frame was rotated (see
/// [`PageRotations`]): there the items live in the turned frame and this
/// bound belongs to the upright one, so `page_number_bands` reads a top band
/// no item can ever be inside. That is deliberate. The measured top band is
/// always at or above the calibrated `PAGE_NUMBER_TOP_Y`, so a turned page
/// simply keeps the behaviour it had before this measurement existed —
/// nothing above 720 in its own frame either. Turning the box would start
/// deleting digits from turned pages, which is a different change and has no
/// evidence behind it.
